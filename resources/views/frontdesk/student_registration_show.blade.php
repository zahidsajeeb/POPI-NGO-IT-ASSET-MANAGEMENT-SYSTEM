<!DOCTYPE html>
<html lang="en">
<head>
    @include('admin.include.stylesheet')
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/2.1.4/toastr.min.css">
    <style>

        * {
            font-family: Verdana, Arial, sans-serif;
        }

        a {
            color: #fff;
            text-decoration: none;
        }

        table {
            font-size: 14px;
        }

        tfoot tr td {
            font-weight: bold;
            font-size: 14px;
        }
        .information {
            background-color: #60A7A6;
            color: #FFF;
        }

        .information .logo {
            margin: 5px;
        }

        .information table {
            padding: 10px;
        }
    </style>
</head>
<body>
@include('admin.include.navbar')
<div class="page-content">
    @include('admin.include.sidebar')
    <div class="content-wrapper">
        <div class="content-inner">
            <div class="page-header page-header-light">
                <div class="page-header-content header-elements-lg-inline">
                    <div class="page-title d-flex">
                        <h4><i class="icon-home"></i> <span class="font-weight-semibold">Home</span> - Student Information</h4>
                    </div>
                    <div class="navbar-right">
                        <a href="{{url('pdf_show', $data->id)}}" class="btn btn-sm btn-info" style="border-radius:0px;"><i class="fa fa-print"></i> Print</a>
                        <a href="{{url('student_edit', $data->id)}}" class="btn btn-sm btn-primary" style="border-radius:0px;"><i class="fa fa-edit"></i> Edit</a>
                        <a href="{{url('home')}}" class="btn btn-sm btn-danger" style="border-radius:0px;"><i class="icon icon-backward2"></i> Back to Previous Page</a>
                    </div>
                </div>
            </div>
            <div class="content">
                <div class="row">
                    <div class="col-lg-2"></div>
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-body">
                                <div class="information">
                                    <table width="100%">
                                        <tr style="height:100px;">
                                            <td align="left" style="width: 30%;">
                                                <img src="data:image/jpeg;base64," alt="Logo" width="64" class="logo" style="text-align:center; width:180px; height:80px;"/>
                                            </td>
                                            <td>
                                                <h2 style="font-size:26px; font-weight:bold; margin-top:25px;"> Visa Automation System </h2><br>
                                            </td>
                                        </tr>
                                    </table>
                                </div>
                                <div class="invoice">
                                    <table class="table table-bordered">
                                        <tr>
                                            <td colspan="10" style="background:#083078d4; font-size:18px; padding:15px; font-weight:bold; color:wheat; text-align:center;">Student Information</td>
                                        </tr>
                                        <tr>
                                            <td rowspan="5" style="width:40%; text-align:center;">
                                                <img src="{{ asset('upload/' . $data->student_img) }}" height="250" width="250">
                                            </td>
                                            <td style="width:20%;background:#f7f7f7; font-weight:bold; font-size:15px; text-align:center;">Student Name</td>
                                            <td style="width:40%;text-align:center; font-size:13px; word-break:break-all; word-wrap:break-word;">{{$data->student_name}}</td>
                                        </tr>
                                        <tr>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:15px; text-align:center;">Counselor Name</td>
                                            <td style="text-align:center; font-size:13px; word-break:break-all; word-wrap:break-word;">
                                                @php
                                                    $counselor_name=DB::table('tbl_counselor')->select('counselor_name')->where('counter_no', '=', $data->counselor_name)->first();
                                                @endphp
                                                {{($counselor_name->counselor_name)}}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:15px; text-align:center;">Token</td>
                                            <td style="text-align:center; font-size:13px; word-break:break-all; word-wrap:break-word;">{{$data->student_id}}</td>
                                        </tr>
                                        <tr>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:15px; text-align:center;">Email</td>
                                            <td style="text-align:center; font-size:13px; word-break:break-all; word-wrap:break-word;">{{$data->student_email}}</td>
                                        </tr>
                                        <tr>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:15px; text-align:center;">Create Date/Time</td>
                                            <td style="width:20%;text-align:center; font-size:13px; word-break:break-all; word-wrap:break-word;">{{$data->created_at}}</td>
                                        </tr>
                                    </table>

                                    <table class="table table-bordered" style="width:100%;">
                                        <tr>
                                            <td colspan="10" style="background:#083078d4; font-size:18px; padding:15px; font-weight:bold; color:wheat; text-align:center;">Counseling History</td>
                                        </tr>
                                        <tr>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:16px; text-align:center;">Sl no</td>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:16px; text-align:center;">Counseling Date</td>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:16px; text-align:center;">Counseling Purpose</td>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:16px; text-align:center;">Counselor Name</td>
                                            <td style="background:#f7f7f7; font-weight:bold; font-size:16px; text-align:center;">Status</td>

                                        </tr>
                                        @php
                                            $history = DB::table('student_entry')->select('*')->where('student_id', '=', $data->student_id)->get();
                                        @endphp

                                        @foreach($history as $key=>$result)
                                            <tr>
                                                <td>{{++$key}}</td>
                                                <td style="text-align:center;">{{$result->date}}</td>
                                                <td style="text-align:center;">{{$result->purpose}}</td>
                                                <td style="text-align:center; word-break:break-all; word-wrap:break-word;">
                                                    @php
                                                        $counselor_name=DB::table('tbl_counselor')->select('counselor_name')->where('counter_no', '=', $result->counselor_name)->first();
                                                    @endphp
                                                    {{($counselor_name->counselor_name)}}
                                                </td>
                                                <td>
                                                    @if($result->active=='1')
                                                        Active
                                                    @else
                                                        Inactive
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
                                    </table>
                                </div>
                                <div class="information" style="width:100%; bottom: 0;">
                                    <table class="table table-bordered" style="color:white;">
                                        <tr>
                                            <td align="left" style="width: 50%;">
                                                &copy; Visa BD - All rights reserved.
                                            </td>
                                            <td align="right" style="width: 50%;">
                                                Printing Time: <?php echo date("Y-m-d H:i:s A"); ?>
                                            </td>
                                        </tr>

                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-2"></div>
                </div>
            </div>
            @include('admin.include.footer')
        </div>
    </div>
</div>
</body>
</html>







