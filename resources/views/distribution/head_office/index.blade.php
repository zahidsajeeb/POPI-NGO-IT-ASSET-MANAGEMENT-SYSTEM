                                                ...
                                                <table class="table table-bordered" id="available_item_table" style="border:1px solid; text-align:center;width:100% !important;">
                                                    ...
                                                    @foreach($available_items as $key=>$row)
                                                        ...
                                                        <tr>
                                                            <td>{{++$key}}</td>
                                                            <td>
                                                                <a href="{{url('head_office_distribution/distribution', $row->id)}}" class="btn btn-sm btn-teal" style="border-radius:0px;text-align:center;"><i class="fa fa-edit"></i> Distribution</a>
                                                            </td>
...
<script>
    ...
    $(function () {
        ...
        {{--    processing: true,--}}
        {{--    serverSide: true,--}}
        {{--    ajax: "{{ url('head_office_available_item_list') }}",--}}
        {{--    columns: [--}}
    ...
    $('body').on('click', '#delete', function () {
        ...
        if (confirm("Do you really want to delete distribution?")) {
            $.ajax({
                type: "get",
                url: "head_office_distribution/delete/" + id,
                success: function (data) {
                    swal({
                        icon: 'error',
                        title: 'Distribution Delete Successfully !!!',
                        showConfirmButton: true,
                        timer: 2500
                    });
                    $('#head_office_distribution_table').DataTable().ajax.reload();
