    ...
    <div class="modal-dialog modal-xl" style=" border-radius: 0px !important;">
        <div class="modal-content">
            <div class="modal-header bg-indigo text-white" style=" border-radius: 0px !important;">
                <h6 class="modal-title"><i class="fa fa-plus"></i> Branch Servicing Add Form</h6>
...
<div id="modal_head_servicing_add" class="modal fade" tabindex="-1" style=" border-radius: 0px !important;">
    <div class="modal-dialog modal-lg" style=" border-radius: 0px !important;">
        <div class="modal-content">
            <div class="modal-header bg-indigo text-white" style=" border-radius: 0px !important;">
                <h6 class="modal-title"><i class="fa fa-plus"></i> Head office Servicing Add Form</h6>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row">
...
<div id="modal_project_servicing_add" class="modal fade" tabindex="-1" style=" border-radius: 0px !important;">
    <div class="modal-dialog modal-lg" style=" border-radius: 0px !important;">
        <div class="modal-content">
            <div class="modal-header bg-indigo text-white" style=" border-radius: 0px !important;">
                <h6 class="modal-title"><i class="fa fa-plus"></i> Project Servicing Add Form</h6>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control required" name="return_date" required>
                                </div>
                                <div class="col-md-6">
...
<script>
    ...
    $(function () {
        ...
        var table = $('#servicing_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('servicing_list') }}",
            columns: [
        ...
        var branch_item_table = $('#branch_item_list_table').DataTable({
            ...
            "orderCellsTop": true,
            "fixedHeader": true,
            ajax: "{{ url('servicing/branch_item_list') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'action', name: 'action', orderable: false, searchable: true},
                {data: 'asset_code', name: 'asset_code'},
        ...
        var head_item_table = $('#head_item_list_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('servicing/head_item_list') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'action', name: 'action', orderable: false, searchable: true},
                {data: 'asset_code', name: 'asset_code'},
        ...
        var project_item_table = $('#project_item_list_table').DataTable({
            ...
            "orderCellsTop": true,
            "fixedHeader": true,
            ajax: "{{ url('servicing/project_item_list') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'action', name: 'action', orderable: false, searchable: true},
                {data: 'asset_code', name: 'asset_code'},
    ...
    $('body').on('click', '#delete_service', function () {
        ...
        if (confirm("Do you really want to delete service?")) {
            $.ajax({
                type: "get",
                url: "servicing_delete/" + id,
                success: function (data) {
                    swal({
                        icon: 'error',
                        title: 'Servicing delete successfully !!!',
                        showConfirmButton: true,
                        timer: 2500
                    });
                    $('#servicing_table').DataTable().ajax.reload();
    ...
    $('body').on('click', '#distributon', function () {
        var id = $(this).data("id");
        if (confirm("Do you really want to distribution this item?")) {
            $.ajax({
                type: "get",
                url: "distribution/" + id,
                success: function (data) {
                    swal({
                        icon: 'success',
                        title: 'After Service Distribution Successfully Done !!!',
                        showConfirmButton: true,
                        timer: 2500
                    });
